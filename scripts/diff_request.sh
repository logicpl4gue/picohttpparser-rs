...
#
# Builds the C oracle (symbols renamed to c_*) and the Rust staticlib, links
# both into scripts/difftest_request.c, and runs it over
# tests/corpus/request/. The harness compares every observable output of both
# implementations on full buffers plus every prefix (streaming) across header
# caps {0,1,2,3,5,16,64}. Exit 0 iff zero mismatches.
#
