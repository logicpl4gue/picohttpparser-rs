...
| 2 | Chunked decoder with a forged `_state` outside 0–7 | Aborts (assert build) or hangs reswitching forever (NDEBUG build) | `-1` via `unreachable!` + the seam's panic guard; no data copied, only `_total_read` advanced at entry | INTENTIONAL (a library must never hang/abort the host on bad input) | INTENTIONAL | 2026-09-09 | `tests/chunked.rs::forged_state_fails_closed` (`_state = 8` → `-1`, only `_total_read` advanced); unreachable from any zero-init + transition sequence (state lockstep memcmp'd in every differential case). Sibling note: a forged `_hex_count` (e.g. 127) diverges dev (overflow panic → `-1`) vs release (wraps, gate fires ~144 digits later → `-1`); both defined, memory-safe, same contract violation — no separate row warranted. |
...
| 4 | Slowloris gate with `last_len > len` (streaming contract requires `last_len` ≤ current length) | Reads out of bounds (`buf + last_len - 3` past `buf_end` before `CHECK_EOF` can fire) — UB, not a defined result | Defined `-2` (`saturating_sub` + `.get()`, zero reads past `len`) | INTENTIONAL (fail-closed; the unsafe original cannot be matched without replicating its OOB read) | INTENTIONAL | 2026-09-09 | Gate paths covered by every-prefix streaming sweeps in `diff_*.sh`; contract documented in `src/ffi.rs` slowloris notes |
