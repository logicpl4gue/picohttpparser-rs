...
generated by the driver wrapper once the campaign is a named run). The table
below holds retained-log figures only — every number cites the log that
contains it; aggregates across runs say so explicitly.

| Metric | Request | Response | Headers | Chunked | Upstream gate |
|---|---|---|---|---|---|
| Fuzz cases | 100,000 (s7) | 100,000 (s7) | 100,000 (s7) | 200,190 (s7) + 100,060 (s99) + 40,058 (s12345) calls | — |
| Campaign logs | per-entry logs above | | | `target/fuzz/chunked-20260909T021000Z.log`, `chunked-seed99-20260909.log`, `chunked-seed12345-20260909.log` | — |
| Per-entry log | `target/fuzz/fuzz-perentry-request-20260909.log` | `target/fuzz/fuzz-perentry-response-20260909.log` | `target/fuzz/fuzz-perentry-headers-20260909.log` | chunked rows live in the campaign logs below | — |
